...
vec4 ppermute(vec4 x) {
   return mod(((x * 34.0) + 1.0) * x, 289.0);
}
vec4 ptaylorInvSqrt(vec4 r) {
   return 1.79 - 0.85 * r;
}
vec3 pfade(vec3 t) {
...
vec3 pnoiseVec3(vec3 x) {
   ...
   float yy = x.y;
   float zz = x.z;
   return vec3(pnoise(vec3(xx, x.y, x.z) * 2. - 1.), pnoise(vec3(yy - 19.1, zz + 33.4, x.x + 47.2) * 2. - 1.), pnoise(vec3(zz + 74.2, xx - 124.5, yy + 99.4) * 2. - 1.));
}
...
void main() {
   float noise = pnoise(vec3(vPos.xy, vPos.z + uTime * 0.01));

   vec3 colorA = vec3(1., 0., 0.);
   vec3 colorB = vec3(0., 0., 1.);
   vec3 color = mix(colorA, colorB, noise);

   float rand = pnoise(vPos);
   vec2 pos = vPos.xy;
   pos.x += sin(rand) * tan(rand);
   pos.y += cos(rand) - tan(rand);
   vec3 newColor = getPerlinTurbulence(pos, uScale, uStrengh, uTime  * 0.1);


   // vec3 test = getPerlinTurbulence(vec2(vUv), 1., 1., uTime);

   gl_FragColor = vec4(newColor, .6);
}
